...
// Form logic is refactored into a component to allow Suspense wrapping
function BitlinkForm() {
  ...
  return (
    <div className='bg-[#780016] w-[100vw] h-[100vh] absolute top-0 flex justify-center items-center text-white'>
      <div className='flex flex-col justify-center items-center gap-4 w-[50vw] h-[50vh] mt-30'>
        <ToastContainer />
        <h1 className='heading text-2xl font-bold text-white flex items-start w-[30vw] mb-3'>
          Create your Bittree
        </h1>
        <div className='Handle flex flex-col justify-center items-start gap-2 w-[30vw]'>
          ...
          <input
            ...
            className='bg-white text-gray-950 border focus:outline-[#780016] py-1.5 px-4 rounded-full ml-3 w-[30vw]'
          />
        </div>
        <div className='flex flex-col justify-center items-start gap-2 w-[30vw]'>
          ...
          {links && links.map((item, index) => (
            <div key={index} className='flex justify-center items-center gap-1 w-[30vw] ml-3'>
              <input
                ...
                className='bg-white text-gray-950 border focus:outline-[#780016] py-1.5 px-4 rounded-full w-[15vw]'
              />
              <input
                type="text"
                value={item.linkname}
          ...
          <button
            disabled={links.some(link => !link.link || !link.linkname)}
            onClick={addLink}
            className='bg-green-500 w-[30vw] flex justify-center items-center py-3 mt-4 rounded-full ml-3 cursor-pointer disabled:bg-gray-300 disabled:cursor-default'
          >
            Add Link
          </button>
        </div>
        <div className='flex flex-col justify-center items-start gap-2 w-[30vw]'>
          ...
          <button
            disabled={
              ...
              !profilepic ||
              !description ||
              links.some(link => !link.link || !link.linkname)
            }
            onClick={handleCreateBitlink}
            className='bg-green-500 w-[30vw] flex justify-center items-center py-3 mt-4 rounded-full ml-3 cursor-pointer disabled:bg-gray-300 disabled:cursor-default'
          >
            Create Your Bitlink
          </button>
      ...
      <div>
        <Link href="/pricing">
          <Image
            ...
            src="/product.png"
            alt="product Page"
            className='w-[40vw] h-[90vh] pt-[15vh]'
            srcSet=""
          />
        </Link>
      </div>
